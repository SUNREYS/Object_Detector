...
from eval_plots import (
    ...
    plot_example_crops, plot_fp_examples,
)
...
PI_GAN_DATASET_ROOT = os.path.join(PROJECT_ROOT, "datasets", "PI-GAN")
...
# Modalities generated by image translation (used for FID etc.)
SYNTHETIC_MODALITIES = {"pid", "PI-GAN_gen"}

# Modalities that compare against thermal GT for SSIM / LPIPS / ghost score
# Visible is real but we still compare against thermal as a cross-domain baseline
THERMAL_COMPARISON_MODALITIES = {"pid", "PI-GAN_gen", "visible"}
...
def run_evaluation(
    ...
    use_thermal  = modality.lower() in THERMAL_COMPARISON_MODALITIES
    ...
    elif modality.lower() == "pi-gan_gen":
        val_img_dir = os.path.join(PI_GAN_DATASET_ROOT, "val", "images")
        labels_dir = os.path.join(DATASET_ROOT, "labels", "val")
    else:
        val_img_dir = os.path.join(DATASET_ROOT, "images", modality, "val")
        # Labels are shared across KAIST modalities (same GT, different images)
        labels_dir = os.path.join(DATASET_ROOT, "labels", "val")
    ...
    for img_file in img_files:
        ...
        # Load thermal GT for val set (synthetic + visible cross-domain comparison)
        thermal_bgr = None
        if use_thermal:
        ...
        for fp_info in ev["fp_boxes"]:
            ...
            if img_bgr is not None:
                ...
                # Normalised bbox coords for drawing later
                fp_row["fp_cx"] = (fx1 + fx2) / 2 / img_w
                fp_row["fp_cy"] = (fy1 + fy2) / 2 / img_h
                fp_row["fp_w"]  = (fx2 - fx1) / img_w
                fp_row["fp_h"]  = (fy2 - fy1) / img_h
    ...
    # Example crops for thesis — each modality uses its own images
    example_img_dir = val_img_dir

    thermal_gt_val = os.path.join(THERMAL_GT_DIR, "val") if use_thermal else None
    plot_example_crops(
        df_obj,
        image_dir=example_img_dir,
        output_dir=os.path.join(output_dir, "examples"),
        thermal_dir=thermal_gt_val,
        is_synthetic=use_thermal,
        n_examples=3,
        report_dir=os.path.join(output_dir, "images_report"),
        modality=modality,
    )

    # FP examples — images where model detected something that wasn't a pedestrian
    if not df_fp.empty and "fp_cx" in df_fp.columns:
        plot_fp_examples(
            df_fp,
            image_dir=example_img_dir,
            report_dir=os.path.join(output_dir, "images_report"),
